...
class ControllerExtensionPaymentPagarMeBoleto extends Controller {
    ...
    public function index() {
        ...
        $data['breadcrumbs'][] = array(
            'href' => $this->url->link('extension/payment/pagar_me_boleto', 'token=' . $this->session->data['token'], 'SSL'),
            'text' => $this->language->get('heading_title'),
            'separator' => ' :: '
        );

        $data['action'] = $this->url->link('extension/payment/pagar_me_boleto', 'token=' . $this->session->data['token'], 'SSL');
	    ...
	    if ( isset( $this->request->post['pagar_me_boleto_order_refunded'] ) ) {
		    $data['pagar_me_boleto_order_refunded'] = $this->request->post['pagar_me_boleto_order_refunded'];
	    } else {
		    $data['pagar_me_boleto_order_refunded'] = $this->config->get( 'pagar_me_boleto_order_refunded' );
	    }

	    if ( isset( $this->request->post['pagar_me_boleto_order_pending_refund'] ) ) {
		    $data['pagar_me_boleto_order_pending_refund'] = $this->request->post['pagar_me_boleto_order_pending_refund'];
	    } else {
		    $data['pagar_me_boleto_order_pending_refund'] = $this->config->get( 'pagar_me_boleto_order_pending_refund' );
	    }

	    if ( isset( $this->request->post['pagar_me_boleto_order_refused'] ) ) {
		    $data['pagar_me_boleto_order_refused'] = $this->request->post['pagar_me_boleto_order_refused'];
	    } else {
		    $data['pagar_me_boleto_order_refused'] = $this->config->get( 'pagar_me_boleto_order_refused' );
	    }

	    if ( isset( $this->request->post['pagar_me_boleto_order_processing'] ) ) {
		    $data['pagar_me_boleto_order_processing'] = $this->request->post['pagar_me_boleto_order_processing'];
	    } else {
		    $data['pagar_me_boleto_order_processing'] = $this->config->get( 'pagar_me_boleto_order_processing' );
	    }

	    if ( isset( $this->request->post['pagar_me_boleto_order_authorized'] ) ) {
		    $data['pagar_me_boleto_order_authorized'] = $this->request->post['pagar_me_boleto_order_authorized'];
	    } else {
		    $data['pagar_me_boleto_order_authorized'] = $this->config->get( 'pagar_me_boleto_order_authorized' );
	    }
        ...
        $this->response->setOutput($this->load->view('extension/payment/pagar_me_boleto', $data));
    }

    private function validate() {

        if (!$this->user->hasPermission('modify', 'extension/payment/pagar_me_boleto')) {
    ...
    public function install() {
        ...
        $this->model_extension_payment_pagar_me_boleto->install();
    }

    public function uninstall() {
        $this->load->model('extension/payment/pagar_me_boleto');
        $this->model_extension_payment_pagar_me_boleto->uninstall();
    }
