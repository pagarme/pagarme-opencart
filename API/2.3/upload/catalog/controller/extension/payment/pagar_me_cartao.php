...
class ControllerExtensionPaymentPagarMeCartao extends Controller
{
    ...
    public function index()
    {
        ...
        $data['url2'] = $this->url->link('extension/payment/pagar_me_cartao/error', '', 'SSL');
        ...
        return $this->load->view('extension/payment/pagar_me_cartao', $data);
    ...
    public function confirm()
    {
        ...
        $result = $this->model_extension_payment_pagar_me_cartao->getPagarMeOrderByOrderId($this->session->data['order_id']);
    ...
    public function error()
    {
        ...
        return $this->load->view('extension/payment/pagar_me_cartao_message', $data);
    ...
    public function callback()
    {
        ...
        if ($event == 'transaction_status_changed') {
            ...
            $order_id = $this->model_extension_payment_pagar_me_cartao->getPagarMeOrder($this->request->post['id']);

            $current_status = $this->config->get('pagar_me_cartao_order_' . $this->request->post['current_status']);

            if(!$this->model_extension_payment_pagar_me_cartao->getTotalOrderHistoriesByOrderStatusId($current_status, $order_id)) {
    ...
    public function payment()
    {
        ...
        if ($status == 'paid' || $status == 'processing' || $status == 'authorized' || $status == 'waiting_payment') {
            $this->model_extension_payment_pagar_me_cartao->addTransactionId($this->session->data['order_id'], $id_transacao, $this->request->post['installments'], $this->request->post['bandeira']);

            $json['success'] = true;
        } else {
            $this->model_extension_payment_pagar_me_cartao->addTransactionId($this->session->data['order_id'], $id_transacao);
            $json['success'] = false;
        }
