...
class ControllerExtensionPaymentPagarMeBoleto extends Controller
{
    ...
    public function index()
    {
        ...
        $data['url2'] = $this->url->link('extension/payment/pagar_me_boleto/error', '', 'SSL');
        ...
        return $this->load->view('extension/payment/pagar_me_boleto', $data);
    }
    ...
    public function callback()
    {
        ...
        if ($event == 'transaction_status_changed') {
            ...
            $order_id = $this->model_extension_payment_pagar_me_boleto->getPagarMeOrder($this->request->post['id']);

            $current_status = $this->config->get('pagar_me_boleto_order_' . $this->request->post['current_status']);

            if(!$this->model_extension_payment_pagar_me_boleto->getTotalOrderHistoriesByOrderStatusId($current_status, $order_id)) {
    ...
    public function payment()
    {
        ...
        $transaction = new PagarMe_Transaction(array(
            ...
            'postback_url' => $this->url->link('extension/payment/pagar_me_boleto/callback', '', 'SSL'),
            "customer" => array(
                "name" => $customer_name,
        ...
        if ($status == 'waiting_payment') {
            $this->load->model('extension/payment/pagar_me_boleto');
            $this->model_extension_payment_pagar_me_boleto->addTransactionId($this->session->data['order_id'], $id_transacao, $boleto_url);
