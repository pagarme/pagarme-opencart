...
class ControllerExtensionPaymentPagarMeCheckout extends Controller
{
    ...
    public function index()
    {
        ...
        return $this->load->view('extension/payment/pagar_me_checkout', $data);
    }
    ...
    public function submit()
    {
	    ...
	    $json['postback_url'] = $this->url->link('extension/payment/pagar_me_checkout/callback', '', 'SSL');
    ...
    public function confirm()
    {
        if($this->session->data['payment_method']['code'] == 'pagar_me_checkout') {
            ...
            if ($transaction->status == 'authorized' || $transaction->status == 'paid' || $transaction->status == 'processing') {
                ...
                $this->model_extension_payment_pagar_me_checkout->addTransactionId($this->session->data['order_id'], $transaction->id, NULL, $transaction->installments, $transaction->card->brand);
            } else {
    ...
    public function callback()
    {
        ...
        try {
            ...
            if ($event == 'transaction_status_changed') {
                ...
                if(!$this->model_extension_payment_pagar_me_checkout->getTotalOrderHistoriesByOrderStatusId($current_status, $order_id)) {
