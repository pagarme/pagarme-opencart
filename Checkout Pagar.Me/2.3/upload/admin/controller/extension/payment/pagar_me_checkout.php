...
class ControllerExtensionPaymentPagarMeCheckout extends Controller
{
    ...
    public function index()
    {
        ...
        $data['breadcrumbs'][] = array(
            'href' => $this->url->link('extension/payment/pagar_me_checkout', 'token=' . $this->session->data['token'], 'SSL'),
            'text' => $this->language->get('heading_title'),
            'separator' => ' :: '
        );

        $data['action'] = $this->url->link('extension/payment/pagar_me_checkout', 'token=' . $this->session->data['token'], 'SSL');
        ...
        $this->response->setOutput($this->load->view('extension/payment/pagar_me_checkout.tpl', $data));
    }

    private function validate()
    {

        if (!$this->user->hasPermission('modify', 'extension/payment/pagar_me_checkout')) {
...
//        var_dump($this->error); exit;
    ...
    public function install()
    {
        ...
        $this->model_extension_payment_pagar_me_checkout->install();
    }

    public function uninstall()
    {
        $this->load->model('extension/payment/pagar_me_checkout');
        $this->model_extension_payment_pagar_me_checkout->uninstall();
    }
}
