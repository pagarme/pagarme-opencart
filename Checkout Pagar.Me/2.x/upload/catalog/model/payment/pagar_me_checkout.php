...
class ModelCheckoutOrder extends Model {
    ...
    public function getOrderGAC($order_id) {
        ...
        if ($order_query->num_rows) {
            ...
            return array(
                'pagar_me_checkout_url'=> (isset($order_query->row['pagar_me_checkout_url'])) ? $order_query->row['pagar_me_checkout_url'] : null,
        ...
        if ($order_info) {
            ...
            if (!in_array($order_info['order_status_id'], array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status'))) && in_array($order_status_id, array_merge($this->config->get('config_processing_status'), $this->config->get('config_complete_status')))) {
                ...
                foreach ($order_product_query->rows as $order_product) {
                    ...
                    if($order_product['model'] == 'brinde') {
                        $this->db->query("UPDATE " . DB_PREFIX . "brinde SET quantity = (quantity - " . (int) $order_product['quantity'] . ") WHERE brinde_id = '" . (int) $order_product['product_id'] . "' AND subtract = '1'");
                    }
            ...
            if (!$order_info['order_status_id'] && $order_status_id) {
                ...
                $order_info_custom_fields = $order_info['custom_field'];
                $custom_fields = array();

                if (!empty($order_info_custom_fields)) {
                    $custom_fields_data = $this->model_account_custom_field->getCustomFieldsData('account', $order_info_custom_fields);
                ...
                if (!empty($payment_custom_fields)) {
                    $custom_fields_data = $this->model_account_custom_field->getCustomFieldsData('address', $payment_custom_fields);

                    foreach($custom_fields_data as $custom_field) {
                        $payment_custom_text .=  $custom_field['value'] . ", ";
                    }

                    $payment_custom_text = rtrim($payment_custom_text, ', ');
                }

                if ($order_info['payment_address_format']) {
                    $format = $order_info['payment_address_format'];
                } else {
                    $format = '{firstname} {lastname}' . "\n" . '{company}' . "\n" . '{address_1} {address_custom_fields}' . "\n" . '{address_2}' . "\n" . '{city} {postcode}' . "\n" . '{zone}' . "\n" . '{country}';
                }
                ...
                $replace = array(
                    ...
                    'address_custom_fields' => $payment_custom_text
                );
                ...
                if (!empty($shipping_custom_fields)) {
                    $custom_fields_data = $this->model_account_custom_field->getCustomFieldsData('address', $shipping_custom_fields);

                    foreach($custom_fields_data as $custom_field) {
                        $shipping_custom_text .= $custom_field['value'] . ", ";
                    }

                    $shipping_custom_text = rtrim($shipping_custom_text, ', ');
                }
                ...
                $replace = array(
                    ...
                    'address_custom_fields' => $shipping_custom_text
                );
                ...
                if ($this->config->get('config_order_mail')) {
                    ...
                    $mail->setHtml($html);
                    $mail->setText($text);
                    $mail->setTags('Pedido-Novo-Admin');
                    $mail->send();
            ...
            if ($order_info['order_status_id'] && $order_status_id && $notify) {
                ...
                $mail->setTags('Pedido-Atualizado');
                $mail->send();
            }
        }
