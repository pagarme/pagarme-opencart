...
'" . $this->db->escape($transaction_id) . "'");
    ...
    public function addDescontoBoleto($order_id){
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "order_total SET order_id = '" . (int)$order_id . "', code = 'pagar_me_checkout_desconto', title = 'Desconto do boleto (" . $this->db->escape($desconto) . "%)', `value` = '" . (float)$discountAmount*-1 . "', sort_order = '" . $discount_order . "'");

        /* Atualiza total do pedido */
        $valor_com_desconto = $order['total'] - $discountAmount;
